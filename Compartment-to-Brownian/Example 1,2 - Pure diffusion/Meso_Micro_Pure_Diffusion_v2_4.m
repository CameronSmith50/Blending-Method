...
Initial_Conditions_Pure_Diffusion
...
for l=1:repeats
    ...
    if uniform==1
        %deifne initial position of particles as uniform
    ...
    elseif left==1
        %define initial position as uniform across the left quater
        init_pos=rand(1,n0)*I(1);
        
        %deifne initial position of particles as uniform
        %Calculate initial number of particles
        determ_prop=floor(n0/k)*k;
        rand_init_pos=rand(1,n0-determ_prop)*(1/3);        
        ...
        com=[(3*determ_prop/k)*ones(1,NCOMP),zeros(1,20)]+rand_com;
        
    elseif right==1
        %define initial position as uniform across the right quater
        init_pos=I(2)+(rand(1,n0)*I(1));
    ...
    %initialise brownian particles
    part=[];
    for i=NPART+1:k
        part= [part,H*(i-1) + rand(1,com(i))/(k)];
    end
    ...
    while t<=t_final
        ...
        %find alpha
        alpha=[com(1:(NCOMP+NBLEND)),com(1:(NCOMP+NBLEND))].*d_vec;
...
meso_micro=rec_com_sum/repeats;

Time_MESO_MICRO=toc;

if uniform == 1
    save('./../../Plotting/Data_hybrid_compartment_brownian/TEST_PROBLEM_1/TEST1_BLENDING','meso_micro')
elseif left == 1
    save('./../../Plotting/Data_hybrid_compartment_brownian/TEST_PROBLEM_2/TEST2_BLENDING','meso_micro')
end